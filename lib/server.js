...
var SPDYProxy = function(options) {
  ...
  this.auth = function (handler) {
    ...
    console.log('Loaded external authorization plug-in.'.green);

    self.authentication_handler = handler;
  }
  ...
  function handleRequest(req, res) {
    ...
    if (options.user) {
      self.authentication_handler = function(username, password, next) {
        next(options.username == username && options.password == password)
      }
    }

    if(typeof self.authentication_handler == 'function') {
      ...
      self.authentication_handler(username, password, function(result) {
        if(result)
          return next(req, res);
